...
public:
	...
	virtual void consume( void* data, int dataLen ) {
	}
	;
	// raw data, len in bytes.
};
...
public:
	...
	Resamp( float rateIn, float rateOut, SampleSink* next ) :
			outTimeLeft(1.0), outBufPtr(0), ratio((float) rateIn / (float) rateOut), next(next) {
...
// RDS data.  Send MSB first.  Google search gr_rds_data_encoder.cc to make your own data.
		 };
...
// append to buffer
			...
			for (int i = 0; i < consumable; i++) {
				...
				buffer [i] = ((left [i] + right [i]) / 2 + (left [i] - right [i]) / 2 * sinLut [state * 2]) * 0.9
						+ 0.1 * sinLut [state];
			}
